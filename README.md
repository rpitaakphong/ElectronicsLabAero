...
RC Interactive Learning starts with a controlled filtering example in each category. Tones have zero DC offset; UAV tones use fixed seeded phases shared with the breadboard simulator, while the other examples retain their zero-phase sine waves:

| Category | Useful signal | Interference at 100% strength |
| --- | --- | --- |
| Low-pass: Clean a sensor signal | 200 Hz, 1 V peak | 10 kHz, 0.35 V peak |
| High-pass: Monitor UAV vibration | Motor vibration: 200 Hz, 1 V peak | Aircraft movement: 2 Hz, 0.6 V peak; 5 Hz, 0.3 V peak |
...
The waveform comparison shows the dashed desired reference, measured input, and actual filtered output on common axes. Output amplitude and phase are never corrected to match the reference. **Signal detail** requests four useful periods; **Long overview** requests the longer of that span or one period of the slowest active interference tone (two periods for UAV), revealing baseline movement. Sampling uses at least 32 intervals per fastest active tone, at least 1,001 samples, and at most 32,769. Extreme frequency separation shortens the displayed window with an explicit explanation rather than aliasing. The model sums each tone's analytical complex response, including its input phase, the actual load, and unbuffered interstage loading.
...
Each category remembers its own example values, input selection, and waveform window. Clean/interference switching changes only the added tones. Custom sine/square/triangle controls remain shared across categories; step response stays separate. Reset, route departure, and reload restore defaults.
...
### UAV vibration high-pass example

The high-pass learning category starts with **Vibration + aircraft movement**, representing the voltage output of a conditioned accelerometer: a useful 200 Hz, 1 V-peak motor vibration plus movement at 2 Hz/0.6 V and 5 Hz/0.3 V peak. These values are synthetic teaching examples, not measured flight data. Slow aircraft motion is unwanted specifically when monitoring motor vibration. **Clean motor vibration** removes only the movement; **Custom waveform** retains the ordinary generator controls. Useful frequency/amplitude, movement strength (0–200%), and advanced movement frequencies remain adjustable, including overlapping frequencies that demonstrate the limits of filtering.

The circuit starts with **33 kΩ and 100 nF**, placing cutoff at approximately **48.2 Hz**. The student objective is to retain at least 95% of the 200 Hz component while leaving below 5% of the 2 Hz component and below 15% of the 5 Hz component. The ideal learning circuit retains approximately 97.2%, 4.14%, and 10.31%, respectively. Increasing cutoff improves movement rejection but eventually weakens useful vibration. Plots and the component gain/phase table use volts; coincident tones combine using their phases.

UAV **Signal detail** spans four useful periods (20 ms by default), while **Long overview** starts at 1 second to show two periods of the slowest movement component. The same sampling budget and explicit shortened-window behavior apply at extreme settings. Controls are preserved across category changes and clean/movement switching, and reset on Reset or route departure. Low-pass, band-pass, custom waveforms, and step responses retain their existing behavior.

### Realistic inputs in the RC breadboard simulator

The simulator at `/rc-filter/simulator` offers a student-accessible **Input signal** selector beside the component editor. It switches between the existing function generator, **UAV vibration + aircraft movement**, and **Sensor + high-frequency noise** without changing the circuit, lead positions, or scope settings. No instructor preset password is needed for source selection. The instructor-only **UAV vibration high-pass** preset wires 33 kΩ/100 nF and both probes, enables the UAV source with movement at 100%, and provides a matching reference diagram. It follows the existing preset scope reset and instructs students to press **Autoset**. The general circuit presets retain their original component values.

UAV uses the same amplitudes, frequencies, and seeded phases as Interactive Learning. The sensor uses a 200 Hz, 1 V-peak useful tone plus eight fixed-phase tones at 5/6.5/8/10/12/14.5/17/20 kHz. Their combined long-term noise RMS is 0.25 V at 100% strength. Both examples have independent synthetic output and noise switches and 0–200% noise strength; turning noise off preserves its chosen strength. Repeated acquisitions cannot average their fixed noise away.

For synthetic inputs, the existing **Autoset** takes a fresh acquisition from the current source and wiring, even if the previous record was stopped or its timebase unsupported. It uses 100 ms/div for UAV with movement, 2 ms/div for clean UAV and the sensor, and fits connected enabled channels using actual voltages. It preserves coupling, probe factors, inversion, source settings, and wiring. With no usable signal it leaves the scope and previous capture intact. **Undo Autoset** restores the previous scope and stopped capture. Ordinary function-generator Autoset retains its existing behavior. Manual scope setup remains available; for UAV start with DC coupling and 0.5 V/div, using 100 ms/div for movement or 2 ms/div for vibration detail.

Synthetic sources replace the function generator at MAIN through a modeled 50 Ω buffer and use the existing GEN return and external termination. SFG settings remain intact while its controls and TTL output are inactive. Returning to Function generator restores those settings. The frequency-domain solver calculates each tone through the actual wired network, including loading and common grounds. Both channels share one sampling grid centered on a fixed epoch. It requires at least 32 samples per fastest active period, bounds each record to 24,000 intervals, and reports an unavailable acquisition when time/div is too slow. Mixed signals have no single frequency or phase; cycle/timing/frequency/phase measurements report that limitation, while voltage/RMS/mean/area measurements remain available. Clean UAV and sensor modes restore ordinary single-sine measurements. CH2 Vpp / CH1 Vpp represents filter gain only for a single sine. Scope AC coupling still subtracts only the record mean; use DC coupling to observe movement.

RC saves and custom presets retain version 2, including `rcInput`, while existing version-1 saves load with ordinary generator input. Storage keys stay unchanged. Old EEG experiments retain their original amplified signals and show a conditional **Legacy EEG** choice after recall; it disappears when another source is selected. Legacy EEG Autoset uses 1 s/div with movement or 0.2 s/div without it. Reset restores the blank board and generator input. Packaging includes the source module in both the embedded and offline simulators. Checks cover cross-tool source parity, analytical gain/phase with loading and cascades, sampling limits, save migration, source controls, fresh Autoset and Undo, manual setting preservation, and offline/browser layouts.
